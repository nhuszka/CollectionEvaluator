...
import com.nhuszka.collection_evaluator.evaluator.evaluation_strategy.queue.LinkedListSortingStrategy;
import com.nhuszka.collection_evaluator.evaluator.evaluation_strategy.queue.NewQueueElementAdditionStrategy;
import com.nhuszka.collection_evaluator.evaluator.evaluation_strategy.queue.RandomLinkedListElementAccessAfterSortStrategy;
...
class QueueEvaluator extends CollectionEvaluator {
	...
	QueueEvaluator(Integer numberOfElements) {
		...
		evaluationStrategies.add(new LinkedListSortingStrategy(queue));
		evaluationStrategies.add(new RandomLinkedListElementAccessStrategy(queue));
		evaluationStrategies.add(new RandomLinkedListElementAccessAfterSortStrategy(queue));
